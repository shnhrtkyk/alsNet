...
def main(in_files, density, kNN, out_folder, thinFactor):
    ...
    arch = importlib.import_module("archs.arch4").arch
    model = AlsNetContainer(num_feat=3, num_classes=3, num_points=kNN, output_base=out_folder, arch=arch)
    model.load_model("/mnt/ssd/shino/tm_log/models/model_0/alsNet.ckpt")

    statlist = [["Filename", "StdDev_Classes", "Ground", "NotGround", "Null"]]
    for file_pattern in in_files:
        ...
        for file in glob.glob(file_pattern):
            ...
            basename = os.path.splitext(os.path.basename(d.filename))[0]
            out_folder_tmp = os.path.join(out_folder, basename)
            print(out_folder_tmp)

            if os.path.exists(out_folder_tmp):
            ...
            while True:
                ...
                if points_and_features is not None:
                    # pred
                    dataset.Dataset.Save(out_path, points_and_features[0][idx_to_use], out_path ,
                                         labels=labels[0][idx_to_use], new_classes=None)
                    acc = model.test_single(out_path ,
                                            save_to=out_path,
            ...
            prob_sums = np.zeros((ref_points.shape[0], 3))
            ...
            for filepattern in input_files:
                for predfile in glob.glob(filepattern):
                    input_files.append(predfile)

            for fileidx, batchfile in enumerate(input_files):
                print("Processing file "+ batchfile)
                ds = Dataset(os.path.join(out_folder_tmp, batchfile))
            ...
            # save mearged data
            savename = os.path.join(basepath, d.filename.replace('.las', '_pred.las'))    
            dataset.Dataset.Save(savename, final[:, :3], d.names,
                                 labels=d.labels, new_classes=new_max_class)
            

            np.savetxt(savename.replace('.las', '.txt'), final)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--kNN', default=800000, type=int, required=False, help='how many points per batch [default: 200000]')
