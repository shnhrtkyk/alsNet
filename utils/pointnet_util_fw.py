...
def pointnet_sa_module_2D(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, bn=True, pooling='max', tnet_spec=None, knn=False, use_xyz=True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            ...
            new_points = tf_util.conv2d(new_points, num_out_channel, kernel_size=[1,3],
...
def pointnet_sa_module_1DCNN(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, bn=True, pooling='max', tnet_spec=None, knn=False, use_xyz=True):
    ''' PointNet Set Abstraction (SA) Module
        Input:
            xyz: (batch_size, ndataset, 3) TF tensor
            points: (batch_size, ndataset, channel) TF tensor
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            #new_points = tf.Print(new_points, [new_points], 'new points:', 1, 18)
            new_points = tf_util.conv2d(new_points, num_out_channel, kernel_size=[3,1],
                                        padding='VALID', stride=[1,1],
                                        bn=bn, is_training=is_training,
                                        scope='conv%d'%(i), bn_decay=bn_decay)
            # shp = new_points.shape
